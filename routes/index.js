...
exports.index = function(req, res){
	...
	mongo.connect('mongodb://127.0.0.1:27017/test', function(err, db){
		...
		var collection = db.createCollection('testdb', {'capped':true,'size':10,'max':10}, function(err, col){

			//just insert a new element upon each request
        	col.insert({'accessDate':new Date().toISOString()}, function(err,docs){
	        ...
	        col.find().toArray(function(err, results) {
	        	res.render('index', { title: 'Hello World', dynamiccontent: results});
	        });
